...
router.get(`/:id`, async (req, res) => {
  ...
  if (!product)
    return res.status(500).json({
      sucess: false,
      message: 'There is no any Product with the given id',
    });
  ...
  await Product.findByIdAndDelete(req.params.id)
    .then((product) => {
      if (product) {
        ...
        res.status(400).json({
          success: false,
          message: 'The Product Could not be Founded ',
        });
      }
    })
...
router.put(
  ...
  async (req, res) => {
    ...
    if (!product)
      return res.status(400).send('The product image cannot be updated!!');
    res.send(product);
  }
);
